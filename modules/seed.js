/**
 * LULLABY SEEDED RANDOMNESS (VERSION 3.0)
 * Deterministic PRNG and template utilities for reproducible AI behaviors.
 */
...
/**
 * SeededPRNG: A robust, deterministic random number generator.
 * Uses a Linear Congruential Generator (LCG) with MMIX parameters.
 */
export class SeededPRNG {
    constructor(seed = 0x1337beef) {
        if (typeof seed === 'string') {
            this.state = hashStringToUint32(seed);
        } else {
            this.state = seed >>> 0;
        }
        // Zero-state guard
        if (this.state === 0) this.state = 0x1337beef;
    }
...
export function makeRng(seedUint32) {
    const prng = new SeededPRNG(seedUint32);
    return {
        next: () => prng.next(),
        float01: () => prng.random(),
        pick: (arr) => prng.pick(arr)
    };
}
...
export function makeSeedVars(name, overrides = {}) {
    ...
    const rng = new SeededPRNG(hashStringToUint32(safeName));

    const places = [
        'the old oak tree', 'the back steps', 'the quiet kitchen light',
        'the hallway at night', 'the porch in summer', 'the car with the windows fogged',
        'the late bus stop'
    ];
    const sharedPasts = [
        'hid behind the curtains', 'laughed until our ribs hurt',
        'walked home without speaking', 'made up stories to survive the day',
        'sat in silence and called it a promise'
    ];
    const hideouts = [
        'hide under the stairs', 'duck behind the garage',
        'sit under that old tree', 'share one headphone and stare at the sky'
    ];
    const quotes = [
        "I'm here.", "Don't leave me alone with my head.", "Say it straight.",
        "Let me carry it with you.", "You're not broken. You're tired."
    ];
...
export function generateSeedLines({
    ...
    const rng = new SeededPRNG(hashStringToUint32(`${vars.name}|seed-lines`));
